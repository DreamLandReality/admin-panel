...
import type { Deployment, DeployEvent, DeployStepId, SiteData, Template } from '@/types'
...
function getScreenshotFallback(siteData: SiteData): string | null {
  const source = siteData as Record<string, unknown>
  return getNestedString(source, 'seo.image') ?? getNestedString(source, 'hero.backgroundImage')
}
...
async function captureDeploymentScreenshot(
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      const key = await captureScreenshot(url, deployment.slug)
      if (!key) {
        throw new Error('Screenshot capture returned no object key')
      }

      const { error } = await supabase
        .from('deployments')
        .update({ screenshot_url: key, updated_at: new Date().toISOString() })
        .eq('id', deployment.id)
  ...
  const message = `Screenshot capture failed after ${attempts} attempts; using fallback image`
  log.event('warn', 'deploy.screenshot.failed', message, {
  ...
  try {
    await appendLog(supabase, deployment.id, 'live', 'screenshot', message)
  } catch (error: unknown) {
    log.event('error', 'deploy.status_log.failed', 'Failed to append screenshot status log', {
      deploymentId: deployment.id,
...
async function runStep<T>(
  ...
  try {
    ...
    emit({ step: stepId, status: 'error', message: getErrorMessage(err) })
    throw err
  }
}
...
export async function runDeployPipeline(
  ...
  await runStep('create_repo', 'Creating repository', emit, async () => {
    ...
    log.info(`[Pipeline] create_repo: slug=${slug} repoName=${repoName} templateRepo=${templateRepo} candidate=${candidate}`)
    const exists = await repoExists(candidate)
    log.info(`[Pipeline] create_repo: repoExists(${candidate})=${exists}`)
    if (exists) {
      log.info(`[Pipeline] create_repo: reusing existing repo ${candidate}`)
      ...
      try {
        ...
        if (getErrorMessage(err).includes('422')) {
          log.warn(`[Pipeline] create_repo: 422 on creation, waiting for in-flight generation of ${candidate}`)
          await waitForRepoReady(candidate, 60_000)
          log.info(`[Pipeline] create_repo: in-flight repo now ready, reusing ${candidate}`)
          repoFullName = candidate
          repoUrl = `https://github.com/${candidate}`
          return
        }
  ...
  await runStep('cloudflare_setup', 'Configuring hosting', emit, async () => {
    ...
    if (existing.exists) {
      ...
      try {
        ...
        log.warn(`[Cloudflare] Failed to update SITE_URL for existing project (non-fatal): ${getErrorMessage(error)}`)
      }
    } else {
    ...
    if (result.success) {
      ...
      await supabase
        ...
        .update({
          ...
          screenshot_url: getScreenshotFallback(deployment.site_data),
      ...
      void captureDeploymentScreenshot(supabase, deployment, result.url)
    } else {
